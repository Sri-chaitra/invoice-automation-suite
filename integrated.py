...
class InvoiceApp:
    def __init__(self, root):
        ...
        frame_login = tk.LabelFrame(root, text="Email Login") #helps in keeping a label
        frame_login.pack(fill="x", padx=10, pady=5) #placing the label exactly in geometry app ,padx for horiontal and pady is for vertical
        ...
        tk.Button(frame_login, text="Download PDFs", command=self.start_email_processing).grid(row=0, column=4, padx=10) # button for download pdf

        self.tree = ttk.Treeview(root, show='headings')
        self.tree.pack(fill="both", expand=True, padx=10, pady=5)

        tk.Button(root, text="Export to CSV", command=self.export_csv).pack(pady=10) #export csv helps in exporting the pdf to csv
